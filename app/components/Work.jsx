  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow justify-between cursor-pointer">
        ...
        <p className="text-gray-500 text-sm sm:text-base mb-4 cursor-pointer">{project.period}</p>
        <p className="text-gray-700 text-sm sm:text-base leading-relaxed cursor-pointer">{project.description}</p>
        {project.tags && (
          <div className="flex flex-wrap gap-2 mt-3 cursor-pointer">
            {project.tags.map((tag, index) => (
              <span key={index} className="bg-indigo-100 text-indigo-800 text-xs sm:text-sm px-2 py-1 rounded-full cursor-pointer">
                {tag}
              </span>
            ))}
          </div>
    ...
    >
      ...
      >
        ...
        <div className="p-8 cursor-pointer text-center">
          <h3 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-4 cursor-pointer">{project.title}</h3>
          <p className="text-gray-500 mb-6 cursor-pointer">{project.period}</p>
          <p className="text-gray-700 text-lg sm:text-xl cursor-pointer">{project.description}</p>
          {project.tags && (
...
const Work = () => {
  ...
  return (
    <section id="work" className="w-full px-6 sm:px-10 lg:px-20 py-16 sm:py-20 scroll-mt-24">
      <div className="max-w-7xl mx-auto bg-white/90 backdrop-blur-lg rounded-3xl shadow-2xl p-8 sm:p-12 lg:p-16">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-gray-700 text-base sm:text-lg leading-relaxed">
            I am a Software Engineer from Suhareka, Kosovo with 1+ years of experience in companies like Pabau, Ameba, and O2 — and over 3+ years working on independent projects.
          </p>
        </div>
            ...
            <div className="max-w-md sm:max-w-lg">
              ...
              <div className="flex justify-center items-center gap-10 my-8">
                <div className="text-center">
                  ...
                  <p className="text-gray-500 text-lg sm:text-xl mt-1">Projects</p>
                </div>
              </div>
