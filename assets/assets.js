import user_image from './user-image.png';
import code_icon from './code-icon.png';
import code_icon_dark from './code-icon-dark.png';
import edu_icon from './edu-icon.png';
import edu_icon_dark from './edu-icon-dark.png';
import project_icon from './project-icon.png';
import project_icon_dark from './project-icon-dark.png';
import vscode from './vscode.png';
import firebase from './firebase.png';
import figma from './figma.png';
import git from './git.png';
import mongodb from './mongodb.png';
import img1 from './1.png';
import img2 from './2.png';
import img3 from './3.png';
import img4 from './4.png';
import img7 from './7.png';
import img8 from './8.png';
import img9 from './9.png';
import img10 from './10.png';
import img11 from './11.png';
import img12 from './12.png';
import img13 from './13.png';
import img14 from './14.png';
import img15 from './15.png';
import img16 from './16.png';
import img17 from './17.png';
import img18 from './18.png';
import img20 from './20.png';
import img21 from './21.png';
import img22 from './22.png';
import img23 from './23.png';
import img24 from './24.png';
import img28 from './28.png';
import img29 from './29.png';
import img30 from './30.png';
import img34 from './34.png';
import img35 from './35.png';
import img37 from './37.png';
import img38 from './38.png';
import img39 from './39.png';
import img40 from './40.png';
import img41 from './41.png';
import img42 from './42.png';
import img43 from './43.png';
import img44 from './44.png';
import img45 from './45.png';
import img46 from './46.png';
import img47 from './47.png';
import img50 from './50.png';
import img51 from './51.png';
import img52 from './52.png';
import img53 from './53.png';
import img54 from './54.png';
import img70 from './70.png';
import img71 from './71.png';
import img72 from './72.png';
import img76 from './76.png';
import img75 from './75.png';
import right_arrow_white from './right-arrow-white.png';
import logo from './logo.png';
import mail_icon from './mail_icon.png';
import mail_icon_dark from './mail_icon_dark.png';
import profile_img from './profile-img.png';
import download_icon from './download-icon.png';
import hand_icon from './hand-icon.png';
import header_bg_color from './header-bg-color.png';
import moon_icon from './moon_icon.png';
import sun_icon from './sun_icon.png';
import arrow_icon from './arrow-icon.png';
import arrow_icon_dark from './arrow-icon-dark.png';
import menu_black from './menu-black.png';
import menu_white from './menu-white.png';
import close_black from './close-black.png';
import close_white from './close-white.png';
import web_icon from './web-icon.png';
import mobile_icon from './mobile-icon.png';
import ui_icon from './ui-icon.png';
import graphics_icon from './graphics-icon.png';
import right_arrow from './right-arrow.png';
import send_icon from './send-icon.png';
import right_arrow_bold from './right-arrow-bold.png';
import right_arrow_bold_dark from './right-arrow-bold-dark.png';
import {
  Globe,
  Smartphone,
  Cpu,
  Code,
  Server,
  Briefcase,
  ArrowRight,
  Database,
  Layers,
  Terminal,
  GitBranch,
  Layout,
  Box,
  FileCode,
  Palette,
  Rocket,
  Wrench,
  Cloud,
  Lock,
  Zap,
  Brain,
  ShoppingCart,
  Workflow,
  Network,
  Braces,
  Settings
} from "lucide-react";


export const assets = {
  user_image,
  code_icon,
  code_icon_dark,
  edu_icon,
  edu_icon_dark,
  project_icon,
  project_icon_dark,
  vscode,
  firebase,
  figma,
  git,
  mongodb,
  right_arrow_white,
  logo,
  cv: "/Argjend-Bytyçi-CV.pdf",
  mail_icon,
  mail_icon_dark,
  profile_img,
  download_icon,
  hand_icon,
  header_bg_color,
  moon_icon,
  sun_icon,
  arrow_icon,
  arrow_icon_dark,
  menu_black,
  menu_white,
  close_black,
  close_white,
  web_icon,
  mobile_icon,
  ui_icon,
  graphics_icon,
  right_arrow,
  send_icon,
  right_arrow_bold,
  right_arrow_bold_dark,
};

export const personalInfo = {
  name: "Argjend Bytyçi",
  role: "Software Engineer",
  location: "Suharekë, Kosovo",
  email: "argjendd220@gmail.com",
  phone: "+38349753683",
  linkedin: "https://www.linkedin.com/in/argjend-byty%C3%A7i-485328270",
  tagline: "Fueled by complex challenges and adrenaline.",
};


export const skillData = [
  {
    name: "Next.js",
    type: "Full Stack",
    experience: "1+ years",
    level: "Intermediate",
    description: "Built full-stack apps with REST APIs, GraphQL, Prisma, PostgreSQL, and Firebase — deployed on Vercel.",
    icon: Rocket
  },
  {
    name: "TypeScript",
    type: "Full Stack",
    experience: "5+ months",
    level: "Intermediate",
    description: "Used for type-safe React development in School Management and Rent a Car projects.",
    icon: Braces
  },
  {
    name: "Tailwind CSS",
    type: "Full Stack",
    experience: "1+ year",
    level: "Intermediate",
    description: "Used in modern UI projects such as Portfolio, Rent a Car, and School Management System.",
    icon: Palette
  },
  {
    name: "C#",
    type: "Full Stack",
    experience: "2+ year",
    level: "Intermediate",
    description: "Created Windows desktop apps with .NET and SQLite, like the Pizza Ordering App.",
    icon: FileCode
  },
  {
    name: "GraphQL",
    type: "Full Stack",
    experience: "1+ year",
    level: "Intermediate",
    description: "Integrated with Next.js and HyGraph for optimized data fetching in Rent a Car and Pabau Clinic projects.",
    icon: Network
  },
  {
    name: "REST API",
    type: "Full Stack",
    experience: "2 years",
    level: "Advanced",
    description: "Developed and consumed REST endpoints in PHP, Laravel, and Next.js full-stack apps.",
    icon: Settings
  },
  {
    name: "MySQL",
    type: "Full Stack",
    experience: "4+ years",
    level: "Advanced",
    description: "Used for relational data management in E-commerce, Market Points, and PHP projects.",
    icon: Database
  },
  {
    name: "PostgreSQL",
    type: "Full Stack",
    experience: "1+ years",
    level: "Intermediate",
    description: "Implemented in Pabau Clinic and School Management projects alongside Prisma.",
    icon: Database
  },
  {
    name: "GitHub",
    type: "Full Stack",
    experience: "3+ years",
    level: "Advanced",
    description: "Used for version control, team collaboration, and CI/CD deployment pipelines.",
    icon: GitBranch
  },
  {
    name: "Clerk",
    type: "Full Stack",
    experience: "1+ year",
    level: "Intermediate",
    description: "Used for authentication and user management in AI SaaS and School Management projects.",
    icon: Lock
  },
  {
    name: "HTML",
    type: "Frontend",
    experience: "4+ years",
    level: "Advanced",
    description: "Used for building responsive layouts in multiple projects including Full-Stack E-commerce and Dashboard Generator.",
    icon: Globe
  },
  {
    name: "CSS",
    type: "Frontend",
    experience: "4+ years",
    level: "Advanced",
    description: "Styled responsive web apps using CSS, Bootstrap, and Tailwind CSS.",
    icon: Palette
  },
  {
    name: "JavaScript",
    type: "Frontend",
    experience: "4+ years",
    level: "Advanced",
    description: "Core scripting language for dynamic web apps like Market Points and Rent a Car.",
    icon: Code
  },
  {
    name: "React.js",
    type: "Full Stack",
    experience: "1+ year",
    level: "Intermediate",
    description: "Developed reusable UI components, dashboards, and client interfaces with API integration.",
    icon: Layout
  },
  {
    name: "Bootstrap",
    type: "Frontend",
    experience: "2+ years",
    level: "Advanced",
    description: "Styled Market Points and Ameba apps for responsive and mobile-first layouts.",
    icon: Box
  },
  {
    name: "PHP",
    type: "Backend",
    experience: "4+ years",
    level: "Advanced",
    description: "Developed backend logic, authentication, and APIs for E-commerce and Market Points apps.",
    icon: Server
  },
  {
    name: "Laravel",
    type: "Full Stack",
    experience: "1+ year",
    level: "Medium",
    description: "Built full-stack project management system with React, Inertia, and REST APIs.",
    icon: Workflow
  },
  {
    name: "Java",
    type: "Backend",
    experience: "3+ years",
    level: "Basic",
    description: "Used for OOP and algorithmic problem-solving during academic projects.",
    icon: Cpu
  },
  {
    name: "SQLite",
    type: "Database",
    experience: "2+ year",
    level: "Intermediate",
    description: "Used in C# desktop apps for storing local data efficiently.",
    icon: Database
  },
  {
    name: "Firebase",
    type: "Database / Backend",
    experience: "1+ years",
    level: "Intermediate",
    description: "Used for authentication and real-time storage in AI PDF Chat project.",
    icon: Cloud
  },
  {
    name: "Neon",
    type: "Database / Cloud",
    experience: "2 months",
    level: "Basic",
    description: "Cloud PostgreSQL solution used for AI Healthcare Assistant and modern apps.",
    icon: Cloud
  },
  {
    name: "Prisma",
    type: "Full Stack",
    experience: "Less than 1 year",
    level: "Intermediate",
    description: "Used with PostgreSQL for schema management in School Management System.",
    icon: Layers
  },
  {
    name: "LangChain",
    type: "AI / API",
    experience: "9 months",
    level: "Intermediate",
    description: "Used for AI-based chatbot development and document querying in AI PDF Chat platform.",
    icon: Brain
  },
  {
    name: "Vercel",
    type: "Deploy",
    experience: "1+ year",
    level: "Intermediate",
    description: "Deployment platform for Next.js, AI, and portfolio projects.",
    icon: Rocket
  },
  {
    name: "Figma",
    type: "Design Tool",
    experience: "1 year",
    level: "Intermediate",
    description: "Used for UI/UX prototyping and wireframing before frontend development.",
    icon: Palette
  },
  {
    name: "HyGraph",
    type: "CMS / API",
    experience: "1 year",
    level: "Intermediate",
    description: "Headless CMS used in Rent a Car project with GraphQL API integration.",
    icon: Network
  },
  {
    name: "Jira",
    type: "Working",
    experience: "2+ years",
    level: "Intermediate",
    description: "Used for managing tasks and sprints during freelance and Ameba collaborations.",
    icon: Wrench
  },
  {
    name: "Stripe",
    type: "Full Stack",
    experience: "1+ year",
    level: "Intermediate",
    description: "Integrated secure payment system in AI PDF Chat project.",
    icon: ShoppingCart
  },
  {
    name: "AI",
    type: "Full Stack",
    experience: "1+ year",
    level: "Advanced",
    description: "Built AI-driven tools like Healthcare Assistant and PDF Q&A Chat using LangChain and AssemblyAI.",
    icon: Brain
  }
];

export const languageInterest = [
  {
    name: "Python",
    reason: "Interested in expanding Machine Learning and AI integration skills.",
    icon: Brain
  },
  {
    name: "C++",
    reason: "Improving low-level system and algorithmic programming capabilities.",
    icon: Cpu
  },
  {
    name: "Kotlin",
    reason: "Exploring Android development and modern cross-platform apps.",
    icon: Terminal
  }
];



export const servicesData = [
  {
    icon: <Globe size={36} />,
    title: "Full-Stack Web Development",
    description:
      "I build fast, responsive, and scalable full-stack web applications using Next.js, React, PHP, Laravel, and MySQL. From backend APIs to front-end design — I handle everything end-to-end.",
  },
  {
    icon: <Smartphone size={36} />,
    title: "Desktop & Mobile App Development",
    description:
      "Experienced in creating C# .NET Windows apps and modern responsive mobile web apps that deliver smooth, optimized performance on all devices.",
  },
  {
    icon: <Cpu size={36} />,
    title: "AI & Automation Integration",
    description:
      "Integrating AI tools like ChatGPT, Gemini, and AssemblyAI to automate workflows, analyze data, and enhance user interactions with intelligent features.",
  },
  {
    icon: <Server size={36} />,
    title: "Backend & API Development",
    description:
      "Building and securing APIs with Node.js, PHP, and Laravel. I structure robust databases with MySQL, PostgreSQL, Firebase, and Neon for optimal performance.",
  },
  {
    icon: <Briefcase size={36} />,
    title: "Project Management & Deployment",
    description:
      "Deploying and maintaining applications on Vercel, Firebase, and other platforms. Experienced with Git, Jira, and CI/CD pipelines for smooth project delivery.",
  },
  {
    icon: <Code size={36} />,
    title: "Custom Business Solutions",
    description:
      "Developing tailored web platforms like school management systems, car rental platforms, and market rewards systems adapted to unique business needs.",
  },
];

export const infoList = [
  {
    icon: assets.code_icon,
    iconDark: assets.code_icon_dark,
    title: "Tech Stack",
    description: "HTML, CSS, JavaScript, React.js, Next.js, PHP, MySQL, C#, Laravel, MySQL, Neon, Typescript, GraphQL, OOP",
  },
  {
    icon: assets.edu_icon,
    iconDark: assets.edu_icon_dark,
    title: "Education",
    description: 'Master of Computer Science – University of "Ukshin Hoti" Prizren',
  },
  {
    icon: assets.project_icon,
    iconDark: assets.project_icon_dark,
    title: "Projects",
    description: "Developed 10+ full-stack web and desktop applications with responsive design, always working everywhere where logic is, and continuously exploring game development.",
  }
];

export const projectData = [
  { 
    id: 1, 
    title: "Portfolio Website", 
    period: "10/2025 – Present", 
    description: "Building a portfolio with Next.js, Tailwind CSS, and Framer Motion.",
    tags: ["Next.js", "Tailwind CSS", "Framer Motion"]
  },
  { 
    id: 2, 
    title: "School Management System", 
    period: "11/2025 – Present", 
    description: "Developing a full-stack school management platform using Next.js, TypeScript, Prisma, PostgreSQL, Clerk, and Tailwind CSS.",
    tags: ["Next.js", "TypeScript", "Prisma", "PostgreSQL", "Clerk", "Tailwind CSS"]
  },
  { 
    id: 3, 
    title: "AI Real-Time Healthcare Assistant", 
    period: "09/2025 – 10/2025", 
    description: "Built a real-time AI medical voice agent using Next.js and AssemblyAI for symptom analysis, with Neon DB and Vercel.",
    tags: ["Next.js", "AssemblyAI", "Neon DB", "Vercel"]
  },
  { 
    id: 4, 
    title: "Rent a Car Platform", 
    period: "07/2025 – 08/2025", 
    description: "Car rental platform built with Next.js, React, TypeScript, GraphQL, HyGraph, and Tailwind CSS.",
    tags: ["Next.js", "React", "TypeScript", "GraphQL", "HyGraph", "Tailwind CSS"]
  },
  { 
    id: 5, 
    title: "Project Management Web App", 
    period: "03/2025 – 04/2025", 
    description: "Project management app built using Laravel 11, React, Inertia, and Tailwind CSS.",
    tags: ["Laravel 11", "React", "Inertia", "Tailwind CSS"]
  },
  { 
    id: 6, 
    title: "Pizza Ordering Windows App", 
    period: "02/2025 – 03/2025", 
    description: "Developed a .NET (C#) & SQLite desktop app for order, billing, and sales management.",
    tags: [".NET (C#)", "SQLite", "Windows Forms"]
  },
  { 
    id: 7, 
    title: "AI-Powered PDF Chat Platform", 
    period: "01/2025 – 02/2025", 
    description: "AI PDF Q&A app using Next.js, LangChain, OpenAI, Stripe, and Firebase.",
    tags: ["Next.js", "LangChain", "OpenAI", "Stripe", "Firebase"]
  },
  { 
    id: 8, 
    title: "Market Points Web App", 
    period: "11/2024 – 01/2025", 
    description: "PHP/MySQL rewards platform for redeeming points, integrated with WordPress.",
    tags: ["PHP", "MySQL", "WordPress"]
  },
  { 
    id: 9, 
    title: "Dashboard & Forms Generator", 
    period: "08/2024 – 09/2025", 
    description: "Web app in PHP, HTML, CSS, and JS that generates dashboards and user management tools.",
    tags: ["PHP", "HTML", "CSS", "JavaScript"]
  },
  { 
    id: 10, 
    title: "Full-Stack E-commerce Admin Panel", 
    period: "06/2024 – 07/2024", 
    description: "E-commerce site with admin panel using PHP & MySQLi, product management, and authentication.",
    tags: ["PHP", "MySQLi", "HTML", "CSS", "JavaScript"]
  },
  { 
  id: 11, 
  title: "Python-Based Excel Data Forecasting", 
  period: "01/2023 – 05/2023", 
  description: "Developed a data forecasting solution in Python to analyze and predict weather patterns using advanced time series algorithms such as SARIMA, ARIMA, and NeuralProphet, as well as deep learning models built with Keras. Leveraged Pandas and NumPy for data processing and Excel integration for reporting and visualization.", 
  tags: ["Python", "Pandas", "NumPy", "Machine Learning", "SARIMA", "ARIMA", "NeuralProphet", "Keras"]
},
  {
    id: 12,
    title: "Vue + Laravel Full Stack App",
    period: "07/2024 – 08/2025",
    description: "Built a full stack application with Vue.js frontend and Laravel 11 API backend. Implemented login, registration, protected routes, token-based authentication, image uploads, and deployed both frontend and backend. Used Tailwind CSS for UI and Pinia for state management.",
    tags: ["Vue.js", "Laravel 11", "Tailwind CSS", "Pinia", "API", "Authentication"]
  }
];


export const projectImages = {
  1: [img1, img2, img3],
  2: [img4],
  3: [img7, img8, img9, img34, img35],
  4: [img10, img11, img12, img37],
  5: [img13, img14, img15, img38, img39],
  6: [img16, img17, img18, img40, img41, img42],
  7: [img20, img21, img44, img43],
  8: [img22, img23, img24, img45, img46, img47,img75,img76], 
  9: [],
  10: [img28, img29, img30, img50, img51],
  11: [img70,img71,img72],
  12: [img52, img53, img54],
};


export const trainingData = [
  { title: "AI SaaS Challenge", provider: "PapaReact", period: "01/2025 – 02/2025", description: "Built an AI-powered SaaS project with Clerk authentication and deployment." },
  { title: "Programming Course", provider: "Ylber Veliu (YouTube)", period: "08/2024 – 10/2024", description: "Completed full course on PHP, HTML, CSS, JavaScript, MySQLi, and Laravel." },
  { title: "Forex Trading Course", provider: "Vigan Morina", period: "07/2024 – 08/2024", description: "Studied Forex analysis and risk management strategies." },
  { title: "Following Zerogravitysolutions Tutorials", provider: "YouTube", period: "03/2024 – 04/2024", description: "Learned Git and version control for project collaboration." },
  { title: "C# .NET Course", provider: "Arjan Dalipi | Perprogramera", period: "01/2023 – 07/2023", description: "Practical skills in building Windows apps and understanding OOP principles." },
  { title: "Front-End Course", provider: "SHPIK", period: "04/2023 – 06/2023", description: "Trained in HTML, CSS, Bootstrap, and JavaScript for responsive web design." },
];


export const toolsData = [
  assets.vscode,
  assets.firebase,
  assets.mongodb,
  assets.figma,
  assets.git,
];

export const educationData = [
  {
    title: "Master of Computer Science",
    institution: 'University of "Ukshin Hoti" Prizren',
    startDate: new Date(2023, 9, 1),
    endDate: null,
  },
  {
    title: "Bachelor of Technology in Information and Telecommunication",
    institution: 'University of "Ukshin Hoti" Prizren',
    startDate: new Date(2020, 9, 1),
    endDate: new Date(2023, 8, 30),
  },
  {
    title: "High School Diploma",
    institution: "Gymnasium “Jeta e Re”, Suharekë",
    startDate: new Date(2017, 7, 1),
    endDate: new Date(2020, 7, 31),
  },
  {
    title: "Primary School",
    institution: "Elementary School, Suharekë",
    startDate: new Date(2009, 8, 1),
    endDate: new Date(2017, 5, 30),
    description:
      "During primary school, I was selected as one of the best students.",
  },
];

export const experienceData = [
  {
    company: "Ameba",
    role: "Software Developer",
    type: "Remote - Suharekë, Kosovo",
    startDate: new Date(2024, 9, 1),
    endDate: new Date(2025, 1, 28),
    details: [
      "Built and hosted scalable web solutions using Next.js, JavaScript, PHP, Bootstrap, and MySQLi.",
      "Collaborated via GitHub to maintain clean and reusable code.",
    ],
  },
  {
    company: "Pabau Clinic Software",
    role: "Software Engineer Intern",
    type: "Onsite - Pristina, Kosovo",
    startDate: new Date(2025, 1, 1),
    endDate: new Date(2025, 4, 31),
    details: [
      "Implemented scalable features and optimized functionality for an existing web platform.",
      "Worked with Next.js, React, JavaScript, CSS, GraphQL, and PostgreSQL.",
    ],
  },
  {
    company: "Freelance / Contract / Self-Employed",
    role: "Software Developer",
    startDate: new Date(2023, 9, 1),
    endDate: new Date(2024, 9, 1),
    details: [
      "Developed responsive web applications with Next.js, PHP, MySQLi, and JavaScript.",
      "Handled projects using Jira and GitHub for version control.",
    ],
  },
  {
    company: "Personal Experience",
    role: "Tech Helper (Age 11)",
    type: "Childhood - Suharekë, Kosovo",
    startDate: new Date(2013, 8, 1),
    endDate: new Date(2017, 5, 30),
    details: [
      "At age 11, assisted my dad with his local internet distribution business.",
      "Solved connection and PC issues for his clients around Suharekë.",
      "Gained hands-on experience in networking and troubleshooting at a very young age.",
    ],
  },
];

export const certificateData = [
  {
    title: "Full Stack Developer",
    provider: "Coders Hub",
    startDate: new Date(2024, 2, 1),
    endDate: new Date(2024, 6, 31),
    description:
      "Completed training in HTML, CSS, Bootstrap, JavaScript, PHP, PDO, and Laravel with hands-on projects.",
  },
  {
    title: "AI Bootcamp",
    provider: "Makerspace",
    startDate: new Date(2024, 9, 1),
    endDate: new Date(2024, 10, 30),
    description:
      "AI training on Machine Learning, ChatGPT, Gemini & ShqipAI with practical collaboration.",
  },
];